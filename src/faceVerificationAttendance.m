...
function [averageAccuracy] = faceVerificationAttendance(kEigen, numberOfSubjects,ImageMatrix,LabelMatrix,ImageInEachClass)
 
 accuracy=zeros(numberOfSubjects,1);                                                                                       %initialisation
 correct=zeros(numberOfSubjects,1);
 [thresholdValue]=rocCurve(ImageMatrix, LabelMatrix, ImageInEachClass);
 
 for i=1:numberOfSubjects
    [TrainImage, TestImage , TrainLabel, TestLabel] = holdOneOut(ImageMatrix,LabelMatrix,i); %four fold validation
